...
const basePrinciples = [
  ...
  "如果 context 中有 stockHoldings、fundHoldings、optionHoldings、recentTransactions、marketData，必须优先读取这些字段，不要说拿不到持仓或账单。",
  "stockHoldings 中的 quantity/currentPrice/marketValue 是用户已录入或 App 刷新后的持仓数量、现价和市值；marketData 是 Gateway 实时联网补充的行情数据。",
  "recentTransactions 是脱敏后的历史账单摘要，可以用于分析消费、商家、分类和现金流；不要要求用户重新上传账单，除非该字段为空。",
...
function trimContext(context: Record<string, unknown>, _provider: AIProvider): Record<string, unknown> {
  return {
    ...
    recentTransactions: take(context.recentTransactions, 120),
